...
def get_bottleneck_path(image_lists, label_name, index, bottleneck_dir,
  ...
  Returns:
    File system path string to an image that meets the requested parameters.
  """
  path = get_image_path(image_lists, label_name, index, bottleneck_dir,
                        category) + '.txt'
  ### ADDED IN - look for bottleneck of file originally trained in genus model
  if FLAGS.label_type=='species':
       genus = label_name.split(" ")[0].strip(" ").title()
       path = path.replace(label_name, genus)
  return path    
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default='genus',
      help='Classify by genus or species. If species look for bottleneck file in former genus location'
  )
  parser.add_argument(
      '--output_graph',
      type=str,
